...
def _survive(hours: int, label: str) -> SurvivingHorizonSummary:
    return SurvivingHorizonSummary(hours, label, 2, 2, 1.0, 0.5, 0.10, 0.20, 0.04)


def _weekly(label: str) -> WeeklyRiskSummary:
    return WeeklyRiskSummary(label, 2, 1.0, 0.5, 0.5, 0.0, 0.5, 0.0, 0.0)
...
def test_performance_report_renders_capital_survival_section():
    ...
    report = PerformanceSummary(
        ...
        standard_survivors=tuple(_survive(h, "STANDARD") for h in (24,48,72,168)),
        risky_survivors=tuple(_survive(h, "HIGH+EXTREME") for h in (24,48,72,168)),
    ...
    assert "1x isolated-loss proxy" in all_content
    assert "configured 5× cross-buffer breach" in all_content
    assert "20%-sized acct equiv" in all_content
