    ...
    async def backfill_research_htf_features(
        ...
        *,
        ...
        1. A prior frozen ``run_signals.features`` value from the same episode,
           bounded to one hour before confirmation.
        2. Exact stored MEXC ticker ``rise_fall_rate`` for ``return_24h`` when a
           ticker snapshot exists no more than 30 minutes before confirmation.
        3. Pre-signal 15m/4h candles for previous 1h momentum, 24h-return proxy,
           and 4h EMA20/ATR extension.

        The exact historical full-universe cross-sectional percentile is never
        synthesized from stored ticker rows because only discovery symbols were
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                for row in rows:
                    ...
                    raw_sources = enriched.get("htf_v1_feature_sources")
                    sources = dict(raw_sources) if isinstance(raw_sources, dict) else {}

                    # Existing frozen signal-time values always win.
                    for key in HTF_REQUIRED_FEATURES:
                        if numeric(enriched.get(key)) is not None:
                            sources.setdefault(key, "frozen_signal_snapshot")
                    ...
                    for prior in prior_by_episode.get(episode_id, ()): 
                        ...
                        for key in HTF_REQUIRED_FEATURES:
                            ...
                            if value is not None:
                                enriched[key] = value
                                sources[key] = "prior_run_signal_snapshot"

                    # Stored ticker return is preferable to the candle proxy.
                    ticker = ticker_by_episode.get(episode_id)
                    if numeric(enriched.get("return_24h")) is None and ticker is not None:
                        observed_at = ticker.get("observed_at")
                        age = (
                            (row["confirmed_at"] - observed_at).total_seconds()
                            if isinstance(observed_at, datetime) else None
                        )
                        value = numeric(ticker.get("rise_fall_rate"))
                        if value is not None and age is not None and 0 <= age <= 1800:
                            enriched["return_24h"] = value
                            sources["return_24h"] = "ticker_snapshot"
                            enriched["htf_v1_return_24h_ticker_age_seconds"] = age

                    buckets = candles_by_episode.get(episode_id, {"Min15": [], "Hour4": []})
                    candle_values, candle_sources = reconstruct_candle_htf_features(
                        confirmed_at=row["confirmed_at"],
                        entry_price=numeric(row.get("entry_price")) or 0.0,
                        min15_rows=buckets.get("Min15", ()),
                        hour4_rows=buckets.get("Hour4", ()),
                    )
                    for key, value in candle_values.items():
                        if numeric(enriched.get(key)) is None:
                            enriched[key] = value
                            sources[key] = candle_sources[key]

                    enriched["htf_v1_feature_sources"] = sources
                    enriched["htf_v1_backfill_attempted_at"] = now.isoformat()
                    enriched["htf_v1_backfill_approximate_fields"] = sorted(
                        key for key, source in sources.items() if source == "min15_24h_proxy"
                    )
                    # Recompute tri-state metadata after any recovered raw values.
    ...
    async def sync_research_htf_metadata(self, *, batch_size: int = 1000) -> int:
        ...
        rows = await self.pool.fetch(
            ...
            WHERE NOT (feature_snapshot ? 'htf_v1_version')
               OR COALESCE(feature_snapshot->>'htf_v1_version', '') <> 'htf_unresolved_bull_v1'
               OR NOT (feature_snapshot ? 'htf_v1_computable')
            ORDER BY confirmed_at ASC
            LIMIT $1
            """,
            batch_size,
